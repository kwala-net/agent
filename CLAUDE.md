...
| `app/api/observe/route.ts` | `POST /api/observe` — Kwala cron webhook. Fetches BTC/USD price from Chainlink, calls LLM, records round, opens/closes trades. |
...
|---|---|---|
| `trader-observe-btc.yaml` | Cron every 1 hour | POST to `/api/observe` (no price in payload — server fetches from Chainlink) |
...
All workflows target chainId 11155111 (Ethereum Sepolia). `/api/observe` fetches the BTC/USD price directly from the Chainlink feed (`0x1b44F3514812d835EB1BDB0acB33d3fA3351Ee43`) via `portfolio.getBtcPrice()` — Kwala does not need to pass the price.
